...
PINB = EXPLODEenable();
PINC = EXPLODEenable();
circ1 = CIRCBUFFenable(20, buffer1);
circ2 = CIRCBUFFenable(12, buffer2);

// FPU full access
stm()->func.setreg(&stm()->scb.reg->CPACR, 2, 20, 3);
stm()->func.setreg(&stm()->scb.reg->CPACR, 2, 22, 3);
...
if(zone == 2){ // workspace 1 RTC CALENDAR
	...
	if(PINC.par.LH & (1 << 13)){
		lcd.gotoxy(0,11); lcd.string_size(stm()->func.i16toa(button_press++),9);
	...
	lcd.gotoxy(0,15); lcd.string_size(stm()->func.i32toa(stm()->func.value()),5);
	//lcd.gotoxy(0,15); lcd.string(stm.func.i32toa(test_var));
	//lcd.gotoxy(0,15); lcd.string(stm.func.i32toa(stm.query.SystemClock()));
	//lcd.gotoxy(0,15); lcd.string(stm.func.i32toa(stm.rtc.BckRead(2)));
...
if(zone == 3){ // workspace 2 ADC1 TEMPERATURE
	...
	lcd.gotoxy(1,0);
	lcd.string_size( stm()->func.print_v1(message, 10, "%d %cC", (unsigned int)valuet, (char) 0xDF ), 6);
	//lcd.string_size( stm.func.print_v1(message, 10, "%d %cC", (unsigned int)tempt, (char) 0xDF ), 10);
	//lcd.gotoxy(1,6);
	lcd.string_size( stm()->func.print_v1(message, 10, "%d", (unsigned int)temperature[3], (char) 0xDF ), 10);
} // if
/******************************************************************************/
/******************************************************************************/
...
void calendario(void)
{
	...
	switch(choice){
		case 1: // show time
			...
			rtc()->tr2vec(vecT);
			lcd.gotoxy(3,0);
			lcd.string_size(stm()->func.print_v2("hora: %d%d:%d%d:%d%d", vecT[0],vecT[1],vecT[2],vecT[3],vecT[4],vecT[5]),14);
			value = stm()->func.triggerA(PINC.par.LL,13,rtc()->get_ss());
			if( value > min1 && value < max1 ){
				lcd.clear();
				rtc()->dr2vec(vecD);
				circ1.putstr(&circ1.par, stm()->func.print_v2("data: %d%d:%d%d:20%d%d\r\n", vecD[5],vecD[6],vecD[3],vecD[4],vecD[0],vecD[1]) );
				choice = 2;
			}
		...
		case 2: // show date
			...
			rtc()->dr2vec(vecD);
			lcd.gotoxy(3,0);
			lcd.string_size(stm()->func.print_v2("data: %d%d:%d%d:20%d%d", vecD[5],vecD[6],vecD[3],vecD[4],vecD[0],vecD[1]),16);
			value = stm()->func.triggerA(PINC.par.LL,13,rtc()->get_ss());
			if( value > min1 && value < max1 ){
				lcd.clear();
				rtc()->tr2vec(vecT);
				circ1.putstr(&circ1.par, stm()->func.print_v2("hora: %d%d:%d%d:%d%d\r\n", vecT[0],vecT[1],vecT[2],vecT[3],vecT[4],vecT[5]) );
				choice = 1;
			}
			if( value > min2 && value < max2 ){
				lcd.clear();
		...
		case 4: // Set Hour
			...
			if( value > minp1 && value < maxp1 ){
				...
				if(hour > 23)
					hour = 0;
				lcd.gotoxy(2,0);
				lcd.string_size(stm()->func.print_v2("hora: %d", hour),8);
			}
		...
		case 5: // Set Minute
			...
			if( value > minp1 && value < maxp1 ){
				...
				if(minute > 59)
					minute = 0;
				lcd.gotoxy(2,0);
				lcd.string_size(stm()->func.print_v2("minuto: %d", minute),10);
			}
			if( value > minp2 && value < maxp2 ){
				lcd.clear();
		...
		case 6: // Set Second
			...
			if( value > minp1 && value < maxp1 ){
				...
				if(second > 59)
					second = 0;
				lcd.gotoxy(2,0);
				lcd.string_size(stm()->func.print_v2("segundo: %d", second),11);
			}
			if( value > minp2 && value < maxp2 ){
				lcd.clear();
				circ1.putstr(&circ1.par, "Relogio\r\n");
		...
		case 7: // Set Year
			...
			if( value > minp1 && value < maxp1 ){
				...
				if(ano > 99)
					ano = 0;
				lcd.gotoxy(2,0);
				lcd.string_size(stm()->func.print_v2("Ano: %d", ano),7);
			}
			if( value > minp2 && value < maxp2 ){
				lcd.clear();
		...
		case 8: // Set Month
			...
			if( value > minp1 && value < maxp1 ){
				...
				if(mes > 12)
					mes = 1;
				lcd.gotoxy(2,0);
				lcd.string_size(stm()->func.print_v2("mes: %d", mes),7);
			}
			if( value > minp2 && value < maxp2 ){
				lcd.clear();
		...
		case 9: // Set Day
			...
			if( value > minp1 && value < maxp1 ){
				...
				if(dia > 31)
					dia = 1;
				lcd.gotoxy(2,0);
				lcd.string_size(stm()->func.print_v2("dia: %d", dia),7);
			}
			if( value > minp2 && value < maxp2 ){
				lcd.clear();
				circ1.putstr(&circ1.par, "Data\r\n");
...
void USART1_IRQHandler(void)
{
	...
	if( stm()->usart1.sr.ore() ){ receive = stm()->usart1.get_dr(); }
...
/******************************** COMMENT *************************************
	...
	Overflowing vectors in programs cause very hard bugs to track.
*******************************************************************************/
...
******/
/***
	Cube IDE is much more strict, in the header file scope. It also does not allow global variables
	It likes working more with functional programming and pointers.
